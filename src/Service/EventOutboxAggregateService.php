...
namespace BlackCat\Database\Packages\EventOutbox\Service;

use BlackCat\Core\Database;
use BlackCat\Database\Packages\EventOutbox\Dto\EventOutboxDto;
use BlackCat\Database\Packages\EventOutbox\Mapper\EventOutboxDtoMapper;
 ...
 */
final class EventOutboxAggregateService
{
    use ServiceHelpers;
    ...
    public function __construct(
        private Database $db, private EventOutboxRepository $eventOutboxRepo
    ) {
        // Repository autowiring (generated)
        $this->repositories = [
  'event_outbox' => $this->eventOutboxRepo
];
    }
    ...
    protected function withRowLock(callable $locker, int|string|array $id, callable $fetch, string $mode = 'wait'): mixed
    {
        ...
        return $this->txn(function () use ($locker, $id, $fetch, $mode) {
            ...
            if (!$row) {
                ...
                throw new \BlackCat\Database\Packages\EventOutbox\ModuleException("Record {$idStr} not found for locking.");
            }
